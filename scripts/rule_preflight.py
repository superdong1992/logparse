...
RULES: tuple[Rule, ...] = (
    ...
    Rule(
        rule_id="rules:lifecycle-config",
        doc=f"{RULE_DOC}#ruleslifecycle-config",
        summary="enabled must be an explicit boolean true to enable v2",
        checks=(
            "Missing lifecycle_split.enabled keeps v2 disabled.",
            "enabled must be bool, not a string.",
            "Reliable and multi-instance process sets must be lists.",
        ...
        globs=(
            ...
            "docs/lifecycle-split-v2-*",
        ),
    ),
    Rule(
        rule_id="rules:lifecycle-v2-split",
        doc=f"{RULE_DOC}#ruleslifecycle-v2-split",
        summary="Lifecycle v2 split boundaries must respect origin and effective scopes.",
        checks=(
            "Board-origin boundaries inherit into CPU effective scopes.",
            "CPU-origin boundaries must not create board-origin boundaries.",
            "wide_support is top-level evidence only.",
            "same-PID checks use effective cycle indexes.",
        ),
        globs=(
            "backend/parsing/lifecycle_splitter.py",
            "backend/plugins/mechanisms/module1.py",
            ...
            "backend/query.py",
            "cli.py",
            "tests/test_lifecycle_splitter.py",
            "tests/test_module1_plugin.py",
            "tests/test_cli.py",
            "docs/lifecycle-split-v2-*",
        ),
