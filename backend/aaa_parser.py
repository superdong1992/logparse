...
class AaaParser:
    ...
    def parse_all(self, result: ParseResult) -> dict[str, AaaResult]:
        """解析所有启用的机制模块。返回 {module_name: AaaResult}。"""
        modules = self.config.get_config().mechanism_modules
        results: dict[str, AaaResult] = {}
        for cfg_key, cfg in modules.items():
            if not cfg.enabled or not cfg.module_name:
                continue
            aaa = self._parse_one(result, cfg, cfg_key)
            if aaa:
                results[cfg_key] = aaa
        return results

    def _parse_one(self, result: ParseResult, cfg, cfg_key: str) -> AaaResult | None:
        ...
        aaa_result = AaaResult(module_name=module_name)
        for slot_id, entries in sorted(by_slot.items()):
            slot_output = AaaSlotOutput(slot_id=slot_id)
            slot_output.board_cycles = self._build_cycles(entries, indicator)
            aaa_result.slots.append(slot_output)
    ...
    def _scan_diag(
        ...
        master_kw: re.Pattern | None, name_map: dict[str, str],
        mod_upper: str,
    ) -> list[AaaLogEntry]:
        """扫描诊断日志：Stage 1 模块名预过滤 → Stage 2 完整正则。"""
        entries: list[AaaLogEntry] = []
        ...
        for line in text.splitlines():
            ...
            raw_pname = m.group("ProcessName")
            context = m.group("Context")

            pname, pid = self._parse_pname(raw_pname, name_map)
            ...
            is_active = bool(master_kw and master_kw.search(context))
            ...
            self._dbg(f"[DEBUG] _scan_diag slot={slot} cpu={cpu_id} name={pname} pid={pid} seq={seq} ts={ts} src=diag")
        ...
        for jl in ps.journal_logs:
            ...
            for line in text.splitlines():
                ...
                if pid and indicator and indicator in raw_name.lower():
                    pname = None
                    for diag_n, jnl_n in name_map.items():
                        if jnl_n.lower() == raw_name.lower():
                            pname = diag_n
                            break
                    if pname is None:
                        pname = raw_name
                ...
                self._dbg(f"[DEBUG] _scan_journal slot={ps.slot_id} cpu={ps.cpu_id or '0'} name={pname} pid={pid} seq={seq} ts={ts} src=journal")
    ...
    def _make_cycles(self, entries: list[AaaLogEntry], indicator: str | None = None) -> list[AaaBoardCycle]:
        if not entries:
            return []
    ...
    def _build_processes(self, entries: list[AaaLogEntry]) -> list[AaaProcessLifecycle]:
        by_key: dict[tuple[str, str], list[AaaLogEntry]] = defaultdict(list)
        for e in entries:
            by_key[(e.process_name, e.pid)].append(e)

        lifecycles: list[AaaProcessLifecycle] = []
        for (pname, pid), logs in sorted(by_key.items()):
            ...
            lifecycles.append(AaaProcessLifecycle(
                process_name=pname, pid=pid, logs=logs,
                total_count=len(logs), missing_sequences=missing,
            ))
        return lifecycles

    def _parse_pname(self, raw: str, name_map: dict[str, str]) -> tuple[str, str]:
        pid = ""
        pname = raw
        for diag_n, jnl_n in name_map.items():
    ...
    def write_output(self, aaa_result: AaaResult, output_dir: Path) -> Path:
        """落盘：slot/周期/cpu_N(非0时)/{进程名}-{pid}.log"""
        aaa_dir = output_dir / "aaa" / aaa_result.module_name
        aaa_dir.mkdir(parents=True, exist_ok=True)
        for slot in aaa_result.slots:
            for cycle in slot.board_cycles:
                cycle_dir = aaa_dir / f"slot_{slot.slot_id}" / cycle.dir_name
                # 按 cpu_id 分组
                ...
                for cpu_key, procs in cpu_procs.items():
                    ...
                    for proc in procs:
                        ...
                        out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
                        self._dbg(f"[DEBUG] write_output path={out_path} name={proc.process_name} pid={proc.pid} cpu={cpu_id} line_count={len(lines)}")
        return aaa_dir

    def apply_to_identifier(self, aaa_result: AaaResult, result: ParseResult) -> None:
        if not aaa_result.active_master_slots:
            return
        for slot in result.diagnostic_slots:
            if slot.slot_id in aaa_result.active_master_slots:
