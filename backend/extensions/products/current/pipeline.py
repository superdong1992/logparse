"""当前产品旧 Pipeline 兼容引擎；新入口由 ParseService 编排。"""
...
class PipelineStageError(RuntimeError):
    """Fatal current-product stage failure; mechanism isolation happens below it."""
...
class Pipeline:
    """当前产品兼容引擎；正式生命周期由红区 ParseService 管理。
    ...
    步骤:
      ...
      5. Metadata          — 生成当前产品 scan-coverage projection
    """
    ...
    def run(
        ...
        """运行兼容解析引擎；默认工作区位于系统临时目录。"""
        task_id = task_id or source.stem
        retain = (
            bool(self.pipeline_config.get("keep_workspace", False))
            if keep_workspace is None
            else bool(keep_workspace)
        )
        if retain:
            extract_dir = output_dir / task_id / "extracted"
            extract_dir.mkdir(parents=True, exist_ok=True)
            self.last_workspace = extract_dir
        ...
        with tempfile.TemporaryDirectory(prefix=f"logparse-{task_id}-") as temp_root:
            extract_dir = Path(temp_root) / "extracted"
            return self._run_in_workspace(
                source,
                output_dir,
                product=product,
    ...
    def _run_in_workspace(
        ...
        *,
        ...
        def _safe(
            ...
            *,
            ...
            try:
                ...
                if stage_name:
                    self.stage_records.append(
                        {"name": stage_name, "success": False, "metrics": {}}
                    )
            ...
            if stage_name:
                ...
                self.stage_records.append(
                    {"name": stage_name, "success": True, "metrics": metrics}
                )
                self.performance.record_stage(
                    stage_name,
        ...
        _safe("[4/6] 日志解析 (时间戳+周期+机制模块+角色)",
              ...
              fatal=True)
        self.last_scan_batch = getattr(log_parser, "last_scan_batch", None)
        self.last_diagnostics = tuple(
            getattr(log_parser, "last_mechanism_diagnostics", ())
        )
        if verbose:
        ...
        # Step 6: metadata 是固定正式产物，不再受旧配置开关控制。
        meta_path = _safe("[6/6] 元数据生成",
              lambda: self.metadata_gen.generate(
                  result,
                  output_dir / task_id,
                  product=product,
              ),
        ...
        (output_dir / task_id / "mech_modules").mkdir(parents=True, exist_ok=True)

        cleanup_t0 = time.time()
        cleanup_count = (
            self._cleanup_intermediate_files(extract_dir)
