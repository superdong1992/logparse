...
logger = logging.getLogger("backend.plugins.mechanisms.module1")


class Module1Plugin(MechanismPlugin):
    ...
    def execute(self, context: MechanismContext) -> MechanismOutcome:
        ...
        if not isinstance(inputs, CurrentMechanismInput):
            raise TypeError("Module1 requires CurrentMechanismInput")
        try:
            split_config = LifecycleSplitConfig.from_mapping(
                cfg.get("lifecycle_split", {})
            )
        except ValueError as exc:
            return MechanismOutcome(
                diagnostics=(
                    Diagnostic(
                        code="LP_LIFECYCLE_CONFIG_INVALID",
                        message=f"{self.module_key}: {exc}",
                        severity=DiagnosticSeverity.ERROR,
                        stage=f"mechanism.{self.module_key}",
                    ),
                )
        ...
        mech_result = self._build_result(inputs, split_config)
        if mech_result is None:
            return MechanismOutcome()
        return MechanismOutcome(
            result=mech_result,
            role_signals={
                slot_id: "active" for slot_id in mech_result.active_master_slots
            },
        )
    ...
    def _build_result(
        ...
        split_config: LifecycleSplitConfig,
    ) -> MechResult | None:
        cfg = self.config
        ...
        if (journal_re or journal_re2) and journal_keyword:
            ...
            for private_slot in inputs.private_slots:
